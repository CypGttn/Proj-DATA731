...
app.layout = html.Div([
    ...
    dcc.Graph(id='divergence_rapport-graph'),
...
@app.callback(
    [Output('temperature-graph', 'figure'),
     Output('divergence_rapport-graph', 'figure'),
...
def update_graph(selected_city):
    ...
    # Calcul des divergences par saison par rapport à de nos jours pour la ville sélectionnée
    seasons = ["spring", "summer", "autumn", "winter"]
    divergence_rapport_data = []
    ...
    for season in seasons:
        ...
        for year in years:
            ...
            season_divergence_rapports.append(d)
            
            start_date = start_date.replace(year=start_date.year + 1)
            end_date = end_date.replace(year=end_date.year + 1)

        divergence_rapport_data.append(go.Scatter(
            x=list(years),
            y=season_divergence_rapports,
            mode='lines',
            name=season
        ))

    # Préparer le graphique des divergence_rapports
    divergence_rapport_figure = {
        'data': divergence_rapport_data,
        'layout': {
            'title': f"Divergences pour les saisons par rapport aux années 2015 à 2017 à {selected_city}",
            'xaxis': {'title': 'Années'},
    ...
    divergence_figure = {
        ...
        'layout': {
            'title': f"Divergences pour les saisons d'une année à l'autre à {selected_city}",
            'xaxis': {'title': 'Années'},
            'yaxis': {'title': 'Divergence'},
            'template': 'plotly_white'
        }
    ...
    return temperature_figure, divergence_figure, divergence_rapport_figure
